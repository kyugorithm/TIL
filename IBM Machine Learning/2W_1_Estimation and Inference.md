...
이제 베이지안 통계와 빈도주의 통계의 차이점을 논의해 봅시다. 그래서 여기서부터 빈번한 통계로 시작하겠습니다. 빈도 통계학은 한계까지 반복되는 관측과 관련이 있다. 단골 통계학자의 경우, 우리가 추정하려는 확률에 대한 어떤 아이디어도 없이 시작한다는 것입니다. 그래서 이것을 빈도가 높은 사람들과 베이지안 사람들 모두의 사업적 본보기로 연결시키려 합니다. 큐잉 이론과 관련하여 어떻게 작동하는지 생각해 봅시다. 큐잉 이론이 무엇인지 잘 모르는 사람들에게는 큐잉 이론이 많은 기업들에게 중요한 개념입니다. 이 아이디어는 대기열 또는 회선으로 작업하는 것과 해당 대기열의 크기 또는 회선의 크기를 일치시키는 데 필요한 서버 수를 연구하는 것입니다. 따라서 여러분은 식료품점과 얼마나 많은 계산원이 우리의 고객을 적시에 체크아웃하기 위해 필요할지 생각해 볼 수 있습니다. 또는 이 숫자는 모든 웹 페이지 요청에 응답하기 위해 필요한 웹 서버의 수이며, 들어오는 모든 고객 서비스 호출에 필요한 적절한 응답 기호 또는 고객 서비스 담당자를 포함할 수 있습니다. 따라서 빈도주의 접근법에서 우리가 필요한 것은 일정 기간 동안 특정 수의 고객이 올 확률을 추정하는 것이거나 베이지안 접근법에서도 필요하다. 마지막 절에서 포아송 분포를 연구할 고정 기간 동안 얼마나 많은 양이 들어올지 생각해 보십시오. 따라서 공정은 실제 모집단 평균 또는 그 무엇이든 간에 실제 빈도를 가질 수 있습니다. 하지만 우리는 확률을 모형화하는 데 관심이 있습니다. 실험의 반복 횟수만큼 말이죠. 따라서 우리는 실제 주파수에 대한 사전 지식이 없다고 가정합니다. 대신, 만약 우리의 표본이 충분히 크다면, 우리가 충분한 단서나 선을 보았다면, 우리는 정해진 기간과 우리의 포아송 분포의 매개 변수와 특정한 수의 사람들의 확률에 대한 강력한 추정을 할 수 있다는 사실에 의존합니다. 그런 다음 우리는 그 표본이 주어진 우리의 확률의 추정치를 추론할 수 있다. 따라서 우리는 실험을 여러 번 반복하고, 가능한 한 많은 데이터를 빈번히 사용하는 접근 방식을 원합니다. 따라서 빈도주의자의 경우 표본 모집단에 주어진 확률에 대한 고정 값이 있습니다. 우리는 외부 영향 없이 데이터에서 직접 추정치를 도출한다. 우리는 우리의 크기 표본이 실제 모집단 모수를 얼마나 포함했는지에 대한 우리의 추정치에 대한 신뢰도를 제공한다. 다시 말해서, 우리가 더 많은 데이터를 가질수록, 우리는 표본으로 추출한 데이터가 실제로 모집단 모수를 포함했다는 것을 더 확신할 수 있습니다. 일단 우리가 그 추정치를 얻으면, 우리는 그것을 우리의 관찰된 데이터에 그리고 그 파생된 값에 적용할 수 있다.


이제 베이지안 통계의 경우, 모수 자체는 확률 분포를 가질 수 있습니다. 따라서 확률 분포와 우리의 불확실성에 대한 직접적인 추측을 하는 대신, 우리는 확률 분포를 매개 변수 자체에 제공한다. 즉, 데이터가 많을수록 확률 분포가 우리가 가지고 있는 모수 추정치 주위에 더 촘촘하게 분포됩니다.


이제, 베이지안에서는 실험자들이 분포에 대한 그들의 이전 신념을 통합할 수 있도록 허용한다. 빈도가 높은 사람들의 경우, 이용 가능한 데이터만을 기반으로 합니다. 특정 기간 동안 X명의 사람들이 줄을 서 있는 것을 볼 확률에 대해, 우리는 실제로 교육 받은 추측을 할 수 있으며 베이지안에서는 이것을 우리의 이전 분포에 통합할 수 있다.


그런 다음 데이터를 확인한 후 이전 분포가 업데이트됩니다. 그래서 우리는 초기 추측을 했고, 더 많은 데이터를 본 후에, 우리는 우리의 모수 추정치의 분포가 무엇인지 바꿀 수 있습니다. 그런 다음 분포를 업데이트한 후 사후 분포를 갖게 되는데, 이제 이 분포는 추정치가 얼마가 될지에 대한 추측을 업데이트하기 위해 데이터를 통합했다.


이제 우리는 빈도수와 베이지안 통계 모두에 동일한 수학과 공식을 많이 사용한다. 참고할 주요 요소는 본질적으로 해석입니다. 우리가 논의한 바와 같이 실제 모집단 평균에 대해 얼마나 많은 가능성을 가지고 있는지 추정하는 것이지요. 또는 그 모집단 평균에 대한 분포를 생각해 내기도 합니다. 우리는 해석의 차이가 적절한 방법에 따라 더 많은 것을 지적할 것이다. 따라서 이 섹션에서는 추론과 추정의 차이와 때때로 기계 학습이 추정에만 집중할 수 있는 방법에 대해 논의한 반면, 통계적 추론은 기본 데이터 생성 프로세스를 살펴본다. 우리는 모수 모델링이 분포의 가정과 모수 수에 의존하며 비모수 모델이 가정을 강하지 않을 수 있는 모델링의 매개 변수 및 비모수 접근 방식을 논의하였다.
