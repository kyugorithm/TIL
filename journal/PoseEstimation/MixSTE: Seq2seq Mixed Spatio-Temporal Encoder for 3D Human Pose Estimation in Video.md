...
#### 3.1.1 Separate correlation learning
학습된 표현에 효과적인 운동 궤적을 주입하기 위해 동적 시퀀스에 걸쳐 동일한 관절에 대한 상관 관계를 명시적으로 모델링하기 위해 각 관절의 시간적 대응을 고려한다. 이전 방법과 달리, 우리는 모든 신체 관절을 시간 트랜스포머 블록의 토큰으로 취급하지 않는다. 각 관절의 궤적이 개별 토큰 p(R^1 x T x dm)가 되도록 시간 차원에서 서로 다른 관절을 분리하여 서로 평행하게 모델링한다. 시간차원의 관점에서 본체 관절의 다른 운동궤적을 별도로 모델링하여 시간적 상관관계를 보다 잘 표현한다. 관절 분리는 다음과 같이 작동한다.
<img width="577" alt="image" src="https://user-images.githubusercontent.com/40943064/159110342-df233a6d-9942-4682-b7a9-3a52e5cfd5ed.png">

(pi,j ∈ PN,T : j 프레임의 i번째 관절, F : the temporal encoder function, Xl ∈ R^RNxTxdm : l번째  TTB encoder 출력  

각 신체관절을 개별 토큰으로 취급함으로써 모델의 dimensionㅇPoseFormer의 N × dm에서 dm으로 감소시킬 수 있으며, 모델 내에서 처리되는 긴 시퀀스를 가능하게 한다.
