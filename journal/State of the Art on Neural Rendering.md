...
CG와 ML 컨셉에 대한 개요로 시작해서 NR 접근법의 중요한 점을 논의한다.  ![145592173-2aabccb5-6d79-430c-9d15-ece1f645ef52](https://user-images.githubusercontent.com/40943064/146351957-2f06f1b7-54f6-4c2b-ae28-6eef0fb05982.png)
...
안타깝게도 학습된 G는 흐릿한 이미지 또는 그럴듯한 출력에 대한 평균 결과를 생성하는 경향이 있다.  
예를 들어, image colorization에서 학습된 G는 평균화 효과로 인해 때때로 desaturated 결과를 생성한다.  
이미지  super-resolution에서는 p-norm이 각 픽셀을 독립적으로 보기 때문에 G가 구조와 세부 정보를 합성하지 못한다.  

유사성에 대한 인간의 인식과 더 가까운 목적함수를 설계하기 위해, 최근 연구는 사전 학습된  
이미지 classifier F(예: VGG)에 의해 추출된 deep feature representation 사이의 거리 측정을 제안한다.  
이러한 loss는 'p-norm'이 픽셀별 품질을 독립적으로 평가하는 반면, deep feature representation은  
...
이미지 분류 작업을 위해 학습되었기 때문에 다단계 deep feature space에서 일치하는 통계가 인간의 인식과 일치하고  
...
따라서 거리 최소화 대신 deep generative 모델은 distribution matching,  
즉 생성결과의 분포를 학습 데이터 분포에 일치시키는 데 초점을 맞춘다.  
많은 유형의 생성 모델 중에서 GAN은 많은 CV 작업에 대해 유망한 결과를 보여주었다.  
Goodfellow의 원본 작업에서 GAN G G : z → y는 저차원 무작위 벡터 z에서 출력 이미지 y로의 매핑을 학습한다.  
일반적으로 입력 벡터는 multivariate gauss 또는 uniform 분포에서 샘플링한다.  
G는 적대적으로 학습된 D에 의해 "실제"와 구별할 수 없는 출력을 생성하도록 학습된다.  
D는 G에 의해 생성된 합성 이미지를 감지하도록 학습된다.  
얼굴이나 차량과 같은 객체 범주에 대해 학습된 GAN은 객체의 고품질 인스턴스 합성을 배우지만, 일반적으로 합성된 배경은 품질이 낮다.  
...
보다 최근의 연구는 multi-modal 이미지 합성을 위해 latent vector z를 사용한다.  
학습 안정화를 위해, cGAN 기반 방법은 픽셀당 'l1 loss Lrecon(G) (Equation (1)) 및 pereceptual distance loss Lperc(G) (Equation (2))도 채택한다.  
...
장면 외관에 대한 미세 제어는 어려우며 장면 속성 간의 복잡한 non-local 3D interation을 항상 처리할 수는 없다.  
NR은 입력 이미지/비디오에서 새로운 이미지를 제어 가능한 고품질 합성을 가능하게 하기 위해 이러한 접근 방식을 결합할 가능성을 가진다.  
...
중요하게 생각하는 축을 따라 NR 접근법의 분류법을 제안한다.  
  
• Control : Control하고자 하는 것은 무엇이며 어떻게 조절하는가?  
• CG Modules : 어떤 CG 모듈을 활용하며 NR 파이프라인에 어떻게 통합되는가?  
• Explicit/Implicit Control : 매개변수에 대한 explicit 제어를 제공하는가, 아니면 출력으로 얻을 것으로 예상되는 것의 예를 보여줌으로써 implicit으로 수행되는가?  
...
미분가능한 그래픽 모듈을 기반으로 그래픽 지식을 NN에 더 깊이 통합할 수 있다.  
이러한 미분가능한 가능한 모듈은 예를 들어 완전한 컴퓨터 그래픽 렌더러, 3D 회전 또는 조명 모델을 구현할 수 있다.  
이러한 구성 요소는 NN에 물리적으로 영감을 받은 inductive bias를 추가하는 동시에 역전파를 통한 end-to-end 학습을 가능하게 한다.  
...
사전 학습을 통한 분포 사전 학습과 제한된 데이터에 대한 미세 조정은 많은 One-shot 및 Few-shot 합성 시나리오에 유용하다.  
...
3D 재구성의 실제 대응으로 학습되는 가시성 네트워크를 제안한다.  
...
네트워크는 지오메트리와 모양 모두에 대한 감독과 함께 종합적으로 렌더링된 데이터에 대해 학습된다.   
...
GQN은 컨텍스트와 동일한 장면의 다른 관찰이 주어지면 각 관찰의 로그 가능성을 최대화하여 학습된다.  
...
최적화 문제를 다루기 쉽게 만들기 위해 GQN은 학습 시간에 근사 사후값을 사용한다.  
...
결과적으로 제한된 학습 데이터 영역에서 다중 뷰 및 원근 기하학을 발견하지 못한다.  
...
DeepVoxels는 이미지와 외부 및 내부 카메라 매개변수만 제공되는 특정 장면에서 학습되며 명시적인 장면 형상이 필요하지 않다.  
...
모델은 끝에서 끝까지 완전히 구별할 수 있으며 학습 세트에 적용된 2D 재렌더링 손실에 의해서만 감독된다.  
...
이러한 방식으로 관점 변경을 기본적으로 지원하는 무조건적인 GAN은 감독되지 않은 방식으로 학습될 수 있다.  
...
구별 가능하고 학습된 신경 광선 분석기는 이미지와 그 외적 및 내재적 카메라 매개 변수만 주어진 상태에서 종단 간 학습을 받는다.   
