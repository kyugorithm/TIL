...
여기서, 다른 도메인의 control 매개 변수 또는 이미지에서 NN를 조절하여 합성 콘텐츠에 대한 control를 달성할 수 있다.  
최근에는 두 영역이 합쳐져 "NR"으로 연구되고 있다. 이 용어를 사용한 최초의 출판물 중 하나는 GQN이다.  
이는 기계가 표현과 생성 NN를 기반으로 주변 환경을 인식하는 것을 배울 수 있게 한다.  
NN가 장면의 여러 이미지를 입력으로 받아들이고 정확한 occlusion으로 임의 뷰를 출력할 수 있기 때문에  
...
장면 속성의 명시적 재구성은 어렵고 오류가 발생하기 쉬우며 rendering된 콘텐츠에 왜곡이 발생한다.  
이를 위해 이미지 기반 rendering 방법은 간단한 경험론을 사용하여 캡처된 이미지를 결합하여 이러한 문제를 극복한다.  
그러나 복잡한 풍경에서, 이 방법들은 seams나 ghost과 같은 artifact를 보여준다.  
NR은 딥 NN를 사용하여 캡처된 이미지에서 새로운 이미지로의 복잡한 매핑을 학습함으로써 재구성 및 rendering을 모두 처리할 수 있는 가능성을 제공한다.  
...
구체적으로, ML을 통해 고전적인 rendering 파이프라인을 개선할 수 있는 위치와 방법과 학습에 필요한 데이터에 대해 논의하고 있다. 
...
Cameras, lights, surface geometry, material을 포함한 장면 정의를 시뮬레이션된 카메라 이미지로 변환하는 과정을 rendering이라고 한다.  
...
품질과 해상도는 물리적 기반 rendering 기술을 사용하여 달성할 수 있는 것과는 거리가 멀었다.  
...
문제를 해결하기 위해, 최근 연구는 사전 학습된 NN에 의해 구성된 높은 수준의 심층 feature embedding 공간에서  
