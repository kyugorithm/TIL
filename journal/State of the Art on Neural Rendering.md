...
구체적으로, ML을 통해 고전적인 rendering 파이프라인을 개선할 수 있는 위치와 방법과 교육에 필요한 데이터에 대해 논의하고 있다. 
...
NR의 새로운 경향 중 하나는 CG 지식을 네트워크 설계에 통합하는 것이다.  
...
이러한 모듈은 예를 들어 장면의 이미지를 렌더링하고 이를 네트워크에 밀도 조절 입력으로 전달하는 데 사용할 수 있다.  
깊이 맵, 일반 맵, 카메라/세계 공간 위치 맵, albedo 맵, 장면의 분산 렌더링 등과 같은 다양한 채널이 네트워크 입력으로 제공될 수 있다.  
이는 문제를 잘 연구된 설정인 이미지 대 이미지 변환 작업으로 변환하며, 예를 들어 스킵 연결이 있는 심층 조건부 생성 모델로 해결할 수 있다.  
차별화된 그래픽 모듈을 기반으로 그래픽 지식을 네트워크에 더 깊이 통합할 수 있습니다.  
이러한 차별화 가능한 모듈은 예를 들어 완전한 컴퓨터 그래픽 렌더러, 3D 회전 또는 조명 모델을 구현할 수 있다.  
이러한 구성 요소는 네트워크에 물리적으로 영감을 받은 유도 편향을 추가하는 동시에 역전파를 통한 종단 간 교육을 가능하게 한다.  
이는 특히 제한된 교육 데이터만 사용할 수 있는 경우 네트워크 구조에서 세상에 대한 정보를 분석적으로 적용하고  
네트워크 용량을 확보하며 더 나은 일반화로 이어지는 데 사용될 수 있다.
