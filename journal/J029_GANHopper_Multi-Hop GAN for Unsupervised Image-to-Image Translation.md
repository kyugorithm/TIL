...
또한, 다중 hop GAN은 예를 들어 중간 번역기의 수와 아키텍처를 변경하는면에서 확장 가능한 일반적인 "meta idea"를 나타낸다.  
...
추가 연구로 "one to many" 변환 작업에서 동일이미지에 대해 고해상도 출력과 여러 가능한 출력(e.x., colorization)이 생성되었다.  
...
이 방식은 약변환(예: 여름에서 겨울, 낮에서 밤)과 관련된 경우 성공하지만 큰 변형이 필요한 경우(예: 고양이에서 강아지로) 종종 실패한다.  
cycle-consistency를 포함하는 네트워크는 전체 이미지 문맥을 고려하는 D와  
perceptual loss로 학습될 때 더 큰 모양 변화를 수행하는 것으로 나타났다[5].  
...
얼굴을 캐리커처 스케치로 변환하는 영역별 방법은 얼굴 랜드마크를 감지/변형 하고 얼굴을 왜곡하는 데 사용하여 이를 수행한다[2].  
...
**2.3 본 방법론**
우리는 변환이 일련의 smooth hopping으로 수행되는 경우 직접 I2I 변환이 모양 세부 사항을 보존하면서 큰 모양 변경을 생성할 수 있음을 보인다.  
이 과정은 도메인간 보간 시퀀스를 생성하는 것으로 볼 수 있다. 많은 GAN은 latent space에서 선형보간을 통해 이미지 사이에 보간을 생성할 수 있다.  
이러한 보간은 데이터 세트[12]에 지정되거나 자동으로 추론되는[4] 해석 가능한 방향을 따를 수도 있다. 그러나 GAN latent 보간은 교차 영역 보간을 수행하지 않는다.  
Aberman et al. [1] 두 도메인의 이미지에서 해당 지점을 식별하고 이 지점을 입력으로 사용하여 이미지 모핑을 구동하여 교차 도메인 보간을 수행한다[13].  
그러나 이 접근 방식은 source 및 target 도메인 모두에 있는 이미지를 보간해야 하는 반면, 우리의 방법은 소스 이미지만 가져와 가장 일치하는 대상 이미지에 대한 보간을 생성한다.  
...
목표는 x ∈ X에 대해 y'이 Y의 이미지 x에 대응하는 것으로 인식되도록 다른 이미지 y' ∈ Y를 출력하는 변환을 배우는 것이다.  
...
이를 위해 hybrid-D를 새롭게 도입하여 이미지가 두 도메인과의 유사도를 평가하여 점수를 생성하도록 한다.  
또한 smoothness loss를 추가하여 G가 변환이 크지 않도록 hop을 통해 이미지의 점진적인 warping을 장려한다.  
...
세번째는 hybrid D인 DH이다. 그림 2는 학습 동안 다중 hop을 통해 이미지를 번역하는 방법을 보여준다.  
...
각 입력 이미지에 대한 이상적인 h를 자동으로 학습할 수 있는가? 극단적으로 말하자면, source에서 target으로 smooth 보간  
