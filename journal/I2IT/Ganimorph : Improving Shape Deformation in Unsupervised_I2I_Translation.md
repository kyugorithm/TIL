...
모양과 질감이 일관된 변형을 학습하는 능력을 평가하기 위해 어려운 장난감 문제를 만들었다.  
정다각형 영역 X와 변형된 등가 Y의 두 영역을 정의한다(그림 3).  
각 예제 Xs,h,d ∈ X에는 면과 겹쳐진 점의 변형된 행렬과 함께 s ∈ {3 . . 7} 면의 중심 정다각형이 포함되어 있다.  
도트 행렬은 단위 도트 그리드를 취하여 가우시안 랜덤 노멀 2×2 행렬인 h와 R2 의 가우스 법선 벡터인 변위 벡터 d를 통해 변환하여 계산된다.  
...
표 2는 DiscoGAN이 두 도메인 간에 매핑하는 방법을 학습할 수 없으며 데이터 세트의 평균(회색)에 가까운 출력을 생성한다는 것을 보여준다.  
CycleGAN은 일반 공간에서 변형 공간으로 매핑할 때 다각형의 파란색 쪽으로 색조 이동을 생성하고 대부분의 경우 변형에서  
일반 공간으로 매핑할 때 변형되지 않은 도트 매트릭스를 생성하는 로컬 변형만 학습할 수 있다.  
대조적으로, 우리의 접근 방식은 dilated D가 이미지 전체에서 정보를 통합하여 변형을 학습하는 데 훨씬 더 성공적이다.  

_Quantitative Comparison_  
출력은 고도로 변형된 이미지이므로 샘플링을 통해 학습된 변환 매개변수를 추정한다.  
우리는 Ground truth 폴리곤과 변환 후 생성된 폴리곤 이미지에서 500개 포인트 샘플 사이의 Hausdorff 거리를 계산한다.  
유한 점 X와 Y 세트의 경우 d(X, Y) = max(y∈Y) min(x∈X) ||kx−yk|| 이다.  
네트워크에 대해 생성된 220개의 폴리곤 경계에 손으로 주석을 달고 경계를 따라 무작위로 균일하게 샘플링한다.  
