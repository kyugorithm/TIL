...
### 3.2. Color module
목표는 주어진 소스 사진의 색상을 제공된 색상 의도에 전달하면서 타겟 카툰의 색상 뉘앙스를 반영하는 것입니다. CARTOONER는 입력 사진 I^Lab_src와 입력 색상 맵 ¯C^Lab_src를 받아들여 ab-채널 이미지 ˆIab src를 생성하는데, 이는 나중에 texture 디코더에서 생성된 texture map ˆI^L_tgt와 연결됩니다. 제어 조작을 시뮬레이션하기 위해, 우리는 합성 색상 맵, ¯CLab src를 생성합니다(그림 7 참조). 입력 사진 IRGB src가 주어지면, 슈퍼픽셀 알고리즘을 적용하여 초기 색상 맵 CRGB src를 생성합니다. 이 과정 없이는 입력 이미지의 세부 사항이 너무 노이즈가 심해져서 색상 신호로 사용하기에 적합하지 않습니다; 슈퍼픽셀은 노이즈 감소 절차로 사용됩니다. 그런 다음, HSV 증강은 IRGB src와 CRGB src의 색상을 변경하여 색상 조작된 이미지 ¯IRGB src와 ¯CRGB src를 만듭니다. 이들은 Lab 공간으로 변환됩니다. 우리는 입력이나 출력 이미지에 색상 전달을 하는 다른 방법과 달리, 우리의 방법이 충실한 시각적 품질을 달성할 수 없다는 것을 관찰했습니다. HSV 증강은 사용자의 다양한 색상 제어 의도를 반영할 수 있는 간단하지만 효과적인 방법입니다. 이는 HSV의 모든 색상 채널을 무작위로 변경합니다; 색조(hue), 채도(saturation), 밝기(value). 색상 변화가 인지적으로 비현실적인 출력을 생성하는 것을 방지하기 위해, 우리는 색상 증강 전에 L 캐싱 트릭을 추가로 적용합니다. 이는 이미지의 밝기(L)를 캐시하고 증강된 이미지의 밝기를 캐시된 것으로 되돌립니다. 우리는 밝기(V-channel) 대신 밝기(L)를 캐시하는데, V가 L에 간접적으로 영향을 미치기 때문이며, 이는 다양한 카툰화와 관련이 있습니다.  
