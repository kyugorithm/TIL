...
3D 포즈의 경우 MPI-INF3DHP와 Human3.6m을 사용합니다. Human3.6m에서 S9 및 S11을 제외한 모든 과목에 대한 교육 시퀀스를 가져온다.  
우리는 프레임을 샘플링한다. MPIINF-3DHP의 경우 8개의 교육 대상을 모두 고려합니다. 
카메라 시점에 대한 어느 정도의 불변성을 학습하기 위해 5대의 가슴 높이 카메라, 2대의 머리 높이 카메라(아래로 기울어짐) 
및 1대의 무릎 높이 카메라(위쪽)에서 시퀀스를 선택합니다. 샘플링된 프레임 사이에 > 200mm만큼 이동한 관절이 하나 이상 있습니다. 
우리는 선택된 프레임의 70%에 배경, 오클루더(의자), 상체 및 하체 확대의 다양한 조합을 사용합니다. 
사람 중심의 작물로 훈련하고 2개의 스케일(0.7×, 1.0×)에서 이미지 스케일 증대를 사용하여 Human3.6m에 대해 
75k 훈련 샘플 및 MPI-INF-3DHP에 대해 100k 훈련 샘플을 생성합니다. 그림 4는 훈련 데이터의 몇 가지 대표적인 프레임을 보여줍니다. 
일반적으로 고려되는 17개의 관절 외에도 발 끝 위치를 사용합니다. 실제 관절 위치는 높이 정규화 골격(무릎-목 높이 92cm)을 기준으로 합니다. 
우리는 학습을 위해 Caffe 프레임워크를 사용하고 반복이 증가함에 따라 학습률이 감소하는 Adadelta 솔버를 사용합니다.  
