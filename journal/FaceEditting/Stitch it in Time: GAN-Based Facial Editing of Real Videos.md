...
### 3.1. Alignment
Face editing을 위해 사전 학습된 StyleGAN2 모델을 사용한다. 그러나 이 모델은 각 이미지가 사전 처리된 FFHQ 데이터 세트에서 학습되었습니다. 특히, 각 이미지는 얼굴을 중심으로 자르고 정렬되었습니다. 따라서 이미지를 GAN의 잠재 공간으로 성공적으로 반전하려면 유사한 자르기 및 정렬 단계가 필요합니다. 그러나 이 전처리 절차는 추출된 얼굴 랜드마크의 정확한 위치에 민감한 개별 단계(예: 자르기)로 구성됩니다. 이러한 민감성은 시간적 불일치의 출현으로 이어질 수 있으므로 이를 줄이는 것을 목표로 합니다. Fox et al.의 작업에서 영감을 받았습니다. [12], 우리는 랜드마크에 대해 가우스 저역통과 필터를 사용합니다. 우리는 이 평활화가 정렬 단계에 의해 유발된 모든 불일치를 극복하기에 충분하다는 것을 발견했습니다.
