...
## 1. Inroduction
Face swapping은 target에서 나타나는 얼굴을 촘촘하게 대체하고 진짜 같은 결과를 만들기 위해 source에서 target으로 얼굴을 전송하는 작업이다. Face transfer 혹은 puppeteering이라고 불리는 face reenactment는 비디오의 제어 얼굴의 움직임과 표정변형을 사용하여 비디오나 이미지에 나타나는 얼굴의 움직임과 변형을 가이드 하는 것이다. 이전 연구에서 swapping과 reenactment 모두 제어 하는 방법은 없었다. 과거에는 외모를 전달하거나 제어하기 위해 기본 3D 얼굴 표현[46]에 의존했다. 얼굴 모양은 입력 이미지 [44, 42, 35]에서 추정하거나 [35] 고정되었다. 그런 다음 3D 형상을 입력 영상[10]과 정렬하고 강도를 전송(face swap)하거나 표정 및 시야를 제어(reenactment)할 때 프록시로 사용했다.  
()
